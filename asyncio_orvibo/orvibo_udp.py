...
import time
from . import _LOGGER
from .const import (CD_ADD_AND_CONTINUE_WAITING, CD_RETURN_IMMEDIATELY, CD_CONTINUE_WAITING)
...
class OrviboUDP:
    ...
    def check_discovery_packet(data, addr):
        return CD_CONTINUE_WAITING if len(data) < 41 or data[4:6] != (DISCOVERY_ID) else CD_ADD_AND_CONTINUE_WAITING
    ...
    async def subscribe_if_necessary(self, timeout=-1, retry=3):
        ...
        if now - self.time_subscribe > SUBSCRIPTION_TIMEOUT:
            ...
            out_data = await OrviboUDP.protocol(MAGIC + SUBSCRIBE_LEN + SUBSCRIBE_ID + self.mac +
                                                ...
                                                self.hp, self.check_subscription_packet, timeout, retry)
            if out_data:
